...
def sendbill():
    ...
    #start the TLS(Transport Layer security)
    s.starttls()

    #authentication
    s.login('errorfruit@gmail.com','esymjffxcfidrslj')

    #text to be sent and the receiver
    rec=emailu.get()
    sen='errorfruit@gmail.com'
    msg=textl.get('1.0',END)
...
receipt=Label(app,text="RECEIPT", font=("bold",22),bg="darkblue",fg="white",width=8)
receipt.grid(row=0, column=3, padx=20,pady=20)
Button(app,text="Send", font=("bold",14),width=8,bg="yellow",fg="darkblue",command=sendbill).grid(row=0,column=4,pady=20)
    #Meal items Label-------------------------------------------------------------------------------
...
def showmanshprice():    #MANGO SHAKE
    ...
    #Meal items units-----------------------------------------------------------------
nvegb=StringVar()
n_vegb=Entry(app,width=4,textvariable=nvegb)   #vegburger
n_vegb.grid(row=1,column=1,padx=20,pady=5)
n_vegb.insert(0,"0")
n_vegb.configure(validate="focusout",validatecommand=showvegbprice)
...
textl=Text(app,width=32,height=28)
textl.grid(row=1,column=3,rowspan=10,columnspan=4)
...
def view_receipt():
    global textl
    textl.insert(INSERT,back.text.format(nvegb.get(),str(int(nvegb.get())*back.cost_vegb),nchkb.get(),str(int(nchkb.get())*back.cost_chkb),nchsd.get(),str(int(nchsd.get())*back.cost_chsnd),nchksd.get(),str(int(nchksd.get())*back.cost_chsnd),nff.get(),str(int(nff.get())*back.cost_ff),nchkng.get(),str(int(nchkng.get())*back.cost_chkng),ncoke.get(),str(int(ncoke.get())*back.cost_coke),nmilks.get(),str(int(nmilks.get())*back.cost_milks),nlimej.get(),str(int(nlimej.get())*back.cost_limej),nmansh.get(),str(int(nmansh.get())*back.cost_mansh),str(tax),str(subtotal),str(int(tax)+int(subtotal))))
...
cust_mail=Entry(app,textvariable=emailu,width=30,font=("bold",11),validate="focusin",validatecommand=cal_tax_and_subtotal)
cust_mail.grid(row=11,column=1,columnspan=2,pady=20)
Button(app,text="Close",width=8,font=("bold",14),bg="red",fg="white",command=_close).grid(row=12,column=0,pady=20)
Button(app,text="Receipt",width=8,font=("bold",14),bg="darkblue",fg="white",command=view_receipt).grid(row=12,column=1,pady=20)
Button(app,text="Clear",width=8,font=("bold",14),bg="black",fg="white",command=clear).grid(row=12,column=2,pady=20)
